...
(async () => {
  ...
  // Д20 – блок сопроводительного письма
  async function handleCoverLetterBlock(coverLetterText) {
    ...
    toggle.click();
    await new Promise(r => setTimeout(r, settings.modalWaitSec * 1000));
    const textarea = document.querySelector('textarea[data-qa*="letter-input"]');
    if (!textarea) {
  ...
  // Д162 – обработка симпл2
  async function handleSimple2(coverLetterText) {
    await sendLog('info', 'д162 модальное окно вида симпл2(предполагаю)', { url: window.location.href });
    const modal = document.querySelector('[aria-modal="true"][role="dialog"]');
    if (!modal) {
      await sendLog('error', 'д162: модальное окно не найдено', {});
      throw new Error('Модальное окно не найдено');
    }
    const letterInput = modal.querySelector('[data-qa="vacancy-response-popup-form-letter-input"]');
    const resumeSelect = modal.querySelector('[data-qa="resume-title"]');
    const submitBtn = modal.querySelector('[data-qa="vacancy-response-submit-popup"]');
    const found = { letterInput: !!letterInput, resumeSelect: !!resumeSelect, submitBtn: !!submitBtn };
    await sendLog('debug', 'д162 нашел ХХ элементов', { found });
    if (!letterInput || !resumeSelect || !submitBtn) {
      await sendLog('error', 'д162 не все элементы найдены, останов', { found });
      throw new Error('Не все элементы для симпл2 найдены');
    }
    // Вставляем сопроводительное
    letterInput.value = coverLetterText;
    letterInput.dispatchEvent(new Event('input', { bubbles: true }));

    // Проверяем, заблокирована ли кнопка (дусл11)
    const isDisabled = submitBtn.hasAttribute('disabled');
    if (isDisabled) {
      await sendLog('debug', 'дусл11 кнопка отклика заблокирована', {});
      // Согласно UML – если дусл11 = да, то идём к д165 и stop
      await sendLog('error', 'д165 не разобрался с кнопкой', { url: window.location.href });
      throw new Error('STOP_EXTENSION');
    } else {
      await sendLog('debug', 'дусл11 кнопка отклика активна', {});
    }
  }

  async function processResumeSelection(vacancyTitle) {
    ...
    const vacancyTitleText = vacancyTitleElement ? vacancyTitleElement.textContent.trim() : vacancyTitle;
    if (!vacancyTitleText) {
    ...
    // ДАлг11 – выбор резюме по названию вакансии
    let selectedResume = null;
    const vacancyLower = vacancyTitleText.toLowerCase();
    ...
    await sendLog('info', 'д17 выбрал резюме', { vacancy: vacancyTitleText, selected: selectedResume });
  ...
  async function sendResponse(modalTypeLabel = '') {
    const modal = document.querySelector('[aria-modal="true"][role="dialog"]');
    if (!modal) {
    ...
    submitBtn.click();
    await sendLog('info', `д18 отправил отклик ${modalTypeLabel}`, { url: window.location.href });
  ...
  try {
    ...
    if (hhState === 'collect_resumes') {
      ...
      // дусл51 – тест
      const testVacancy = settings.test_vacancy;
      const isTest = testVacancy && testVacancy.trim() !== '';
      if (isTest) {
        await sendLog('debug', 'д140 обнаружен дебаг', { url: testVacancy });

        // Формируем список из одной вакансии и переходим сразу в цикл отклика
        const testVacancyObj = {
          url: testVacancy,
          title: 'Тестовая вакансия',
          porNum: 1
        };
        await chrome.storage.local.set({
          multipleVacanciesList: [testVacancyObj],
    ...
    if (hhState === 'processing_vacancies') {
      ...
      if (!vacanciesToProcess || currentVacancyIndex >= vacanciesToProcess.length || currentVacancyIndex >= maxVacanciesToProcess) {
        ...
        // Открываем первую вакансию через д25 (согласно UML)
        const firstVacancy = multipleList[0];
        await sendLog('debug', 'д25 открываю очередную вакансию', { vacancy: firstVacancy });
    ...
    if (hhState === 'process_multiple_vacancies') {
      ...
      // Если мы на странице ответа (vacancy_response) – просто переходим к следующей вакансии
      ...
      // Если мы не на странице этой вакансии – открываем её через д25
      ...
      if (buttonText === 'откликнуться') {
        ...
        // дусл3 – ожидание перехода или модального окна
        let waitResult = null;
        const maxWait = (settings.waitForResponseSec || 10) + 5; // 15 сек
        for (let i = 0; i < maxWait * 10; i++) {
          await new Promise(r => setTimeout(r, 100));
          if (window.location.href.includes('startedWithQuestion')) {
            waitResult = 'dopy';
            break;
          }
        ...
        if (!waitResult) {
          await sendLog('debug', 'дусл3-хз', { url: window.location.href });
          await sendLog('info', 'д40 хрень', { url: window.location.href });
        ...
        if (waitResult === 'success') {
          // Отклик уже отправлен, переходим к следующей
          const newIndex = currentMultipleIndex + 1;
          await chrome.storage.local.set({ currentMultipleIndex: newIndex });
          const currentTabId = await getCurrentTabId();
          if (currentTabId) await closeVacancyTab(currentTabId);
        ...
        if (waitResult === 'dopy') {
          ...
          // Собираем поля (код не меняется, он полный в предыдущей версии)
          // ... (оставляем существующий код для Д15)
          // После заполнения – переход к следующей вакансии
          const newIndex = currentMultipleIndex + 1;
          await chrome.storage.local.set({ currentMultipleIndex: newIndex });
          const currentTabId = await getCurrentTabId();
          if (currentTabId) await closeVacancyTab(currentTabId);
        ...
        if (waitResult === 'mod') {
          ...
          if (modalType === 'simple1') {
            ...
            await handleSimple2(settings.coverLetter);
          }
          const vacancyTitle = document.querySelector('[data-qa="vacancy-title"]')?.textContent.trim() || '';
          await processResumeSelection(vacancyTitle);
          await sendResponse(modalType === 'simple1' ? 'симпл1' : 'симпл2');
    ...
    console.error(err);
    if (err.message !== 'STOP_EXTENSION' && err.message !== 'STOP_EXTENSION') {
      try {
